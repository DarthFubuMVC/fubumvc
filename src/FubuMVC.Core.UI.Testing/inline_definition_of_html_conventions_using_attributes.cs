...
namespace FubuMVC.Core.UI.Testing
{
    ...
    public class inline_definition_of_html_conventions_using_attributes : HtmlElementConventionsContext
    {
        ...
        {
            theRegistry.Displays.IfPropertyHasAttribute<FakeRequiredAttribute>().BuildBy(req => new HtmlTag("b").Text(req.StringValue()));
        ...
        {
            theRegistry.Displays.AddClassForAttribute<FakeRequiredAttribute>("required");
        ...
        {
            theRegistry.Editors.ModifyForAttribute<FakeMaximumStringLength>((tag, att) => tag.Attr("data-max-length", att.MaxLength));
        ...
        {
            theRegistry.Editors.ModifyForAttribute<FakeRequiredAttribute>(tag => tag.AddClass("required"));
