...
namespace FubuMVC.Tests.Registration.Conventions
{
    ...
    public class RedirectableHandlerConventionTester
    {
        ...
        public void SetUp()
        {
            graph = new FubuRegistry(x => x.Actions.IncludeType<RedirectableHandlerController>())
                .BuildGraph();
        ...
        public class RedirectableHandlerController
        {
            ...
            {
                return new Model1 { Name = "ZeroInOneOut" };
            }

            public Model2 OneInOneOut(Model1 input)
            {
                return new Model2 { Name = input.Name };
            }
        ...
        {
            graph.BehaviorFor<RedirectableHandlerController>(x => x.ZeroInOneOut())
                .Any(x => x is RedirectableNode<Model1>)
                .ShouldBeFalse();
        }
        ...
        {
            graph.BehaviorFor<RedirectableHandlerController>(x => x.GoNext())
                .FirstCall().Next
                .ShouldBeOfType<RedirectableNode<Model1>>();
            graph.BehaviorFor<RedirectableHandlerController>(x => x.GoAfter(null))
                .FirstCall().Next
                .ShouldBeOfType<RedirectableNode<Model1>>();
        }
    }
