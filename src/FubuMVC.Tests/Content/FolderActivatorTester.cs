...
namespace FubuMVC.Tests.Content
{
    ...
    public class PackageFolderActivatorTester : InteractionContext<PackageFolderActivator>
    {
        ...
        {
            MockFor<IPackagedImageUrlResolver>().AssertWasCalled(x => x.RegisterDirectory("folder1\\content\\images"));
            MockFor<IPackagedImageUrlResolver>().AssertWasCalled(x => x.RegisterDirectory("folder3\\content\\images"));
        }
