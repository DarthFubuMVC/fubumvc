...
namespace FubuMVC.Core.Registration.Conventions
{
    public class ContinuationHandlerConvention : IConfigurationAction
    {
        ...
        {
            graph.Actions()
                .Where(x => x.OutputType().CanBeCastTo<FubuContinuation>())
                .Each(call =>
                {
    ...
    public class RedirectableHandlerConvention : IConfigurationAction
    {
        ...
        {
            graph.Actions()
                .Where(x => x.OutputType().CanBeCastTo<Redirectable>())
                .Each(call =>
                {
                    ...
                    {
                        var genericParameter = outputType.GetGenericArguments().First();
