...
namespace IntegrationTesting.Fixtures.Packages
{
    public class EnvironmentRunnerFixture : Fixture
    {
        ...
        public EnvironmentRunnerFixture()
        {
            ...
            this["EnvironmentLogs"] = VerifySetOf(() => _environmentLogs).Titled(
                "The log entries for checking the environment should be")
                .MatchOn(x => x.Description, x => x.Success, x => x.TraceText);

            this["InstallAndEnvironmentLogs"] = VerifySetOf(() => _allLogs).Titled(
                "The log entries for both installation and checking the environment should be")
                .MatchOn(x => x.Description, x => x.Success, x => x.TraceText);
        }
        ...
        [FormatAs(
            "Installation log entry with description {description} is marked as a failure and should contain the text {trace}"
            )]
        public bool FailedInstallLogForContains(string description, string trace)
        {
            ...
            StoryTellerAssert.Fail(log == null,
                                   () =>
                                   "Could not find the log entry with that description.  Possibles are:\n" +
        ...
        [FormatAs(
            "Check Environment log entry with description {description} is marked as a failure and should contain the text {trace}"
            )]
